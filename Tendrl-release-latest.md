...
- On the Tendrl server, ensure that Etcd data directory is hosted/located on an separate disk (which is not being used by any other process) and graphite/carbon/whisper (time series) data directories are hosted on a separate disk other than the Etcd disk. (The default data directory for etcd is `/var/lib/etcd`. Mount the disk at this location and ensure that the directory is owned by `etcd:etcd` and that it's permissions are `0755`. The corresponding `fstab` entry would also be required.)
